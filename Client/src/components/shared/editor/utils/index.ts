...
import { EditorInstance } from "../types/editorInstance";
...
export const getPrevText = (editor: EditorInstance, position: number) => {
  ...
  const doc = editor.state.doc.copy(fragment).content.content;

  return editor.storage.markdown.manager.serialize(doc) as string;
};
