...
import { LectureSimpleDto } from "@/api/web-api-client";
import dayjs from "dayjs";

interface LectureCardProps {
  lecture: LectureSimpleDto;
  courseId: string;
}
...
export function LectureCard({ lecture, courseId }: LectureCardProps) {
  return (
    <Link
      ...
      params={{ lectureId: lecture.id, courseId }}
    >
      <Card className="hover:border-primary transition-colors cursor-pointer h-full">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground line-clamp-3 mb-4 text-pretty">
            {lecture.description}
          </p>
