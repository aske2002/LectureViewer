...
namespace strejs.Web.Endpoints;

public class Users : EndpointGroupBase
{
    public async Task<Results<Ok, ValidationProblem>> RegisterAsync(ISender sender, CreateUserCommand command)
    {
        var (result, _) = await sender.Send(command);
        return result.ToHttpResult();
    }

    public async Task<Results<ProblemHttpResult, Ok<UserInfoVm>>> GetInfoAsync(ISender sender, ClaimsPrincipal claimsPrincipal)
    {
        try
        {
            var user = await sender.Send(new GetUserInfoQuery { User = claimsPrincipal });
    ...
    public async Task<Ok> SignOutAsync([FromServices] SignInManager<ApplicationUser> signInManager)
    {
    ...
    {
        app.MapGroup(this)
            .MapRestIdentityEndpoints()
            .MapPost(RegisterAsync, "/register")
            .MapPost(SignOutAsync, "/signout")
            .MapGroup("/manage")
                .RequireAuthorization()
                .MapGet(GetInfoAsync, "/info");
    }
}
...
public static class IdentityEndpointExtensions
{
    ...
    public static IEndpointRouteBuilder MapRestIdentityEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        .Add(endpointBuilder =>
        {
            ...
            confirmEmailEndpointName = $"{nameof(Users)}-{finalPattern}";
